“””
train.py — The agent edits this file. Everything is fair game.

Baseline: linear projector (CLS token only) + small GPT decoder.
Metric: val_loss (cross-entropy nats/token). Lower is better.

Run:
python train.py
python train.py > run.log 2>&1   # agent mode
“””
...
LM_DIM       = 256     # transformer hidden dimension
DEPTH        = 4       # number of transformer blocks
N_HEADS      = 4       # attention heads
N_VIS_USED   = 1       # how many CLIP tokens to use (1=CLS only, max=N_VISUAL_TOKENS)
BATCH_SIZE   = 32
LR           = 3e-4
WEIGHT_DECAY = 0.01
WARMUP_STEPS = 300
...
“””
Baseline VLM:
1. Project N_VIS_USED CLIP tokens → LM_DIM  (one token per image token)
2. Embed caption tokens → LM_DIM
3. Concatenate visual prefix + caption → causal transformer
4. Predict next caption token from each position
“””
def **init**(self):
super().**init**()
...
def forward(self, img_features: torch.Tensor, caption_tokens: torch.Tensor) -> torch.Tensor:
    """
    img_features:    [B, N_VISUAL_TOKENS, IMAGE_EMBED_DIM]  float32
    caption_tokens:  [B, MAX_CAPTION_LEN]                   int64
    Returns: scalar loss (cross-entropy, mean over non-PAD tokens)
    """
    # Visual prefix: use first N_VIS_USED tokens from CLIP output
    vis = self.vis_proj(img_features[:, :N_VIS_USED, :])   # [B, N_VIS_USED, LM_DIM]

    # Text embeddings
    txt = self.tok_emb(caption_tokens)                      # [B, T, LM_DIM]

    # Concatenate and add positional embeddings
    x = torch.cat([vis, txt], dim=1)                        # [B, N_VIS_USED+T, LM_DIM]
    x = x + self.pos_emb(torch.arange(x.shape[1], device=x.device))

    # Transformer
    for block in self.blocks:
        x = self.ln_f(block(x)) if block is self.blocks[-1] else block(x)

    # Predict next caption token from visual+caption positions
    # Position (N_VIS_USED - 1) predicts caption token 0,
    # position (N_VIS_USED + i - 1) predicts caption token i.
    logits = self.head(x[:, N_VIS_USED - 1 : N_VIS_USED + MAX_CAPTION_LEN - 1, :])  # [B,T,V]
...
print(f"VLM | params={n_params:.1f}M | depth={DEPTH} | dim={LM_DIM} | "
      f"n_vis={N_VIS_USED} | device={DEVICE}")

# Optional: compile for speed on PyTorch 2+
try:
...
scheduler = torch.optim.lr_scheduler.LambdaLR(
    optimizer, make_lr_lambda(WARMUP_STEPS, 20_000)
)
...
# ── Summary (grep-friendly, fixed keys) ───────────────────────────────────
print("---")
